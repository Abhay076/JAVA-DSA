...
public class MinCostPaths {
	...
	public static int minCostPathsR(int cost[][],int i, int j,int dp[][]) {
		int m=cost.length;
		int n=cost[0].length;
		if(i==m-1&&j==n-1) {
			return cost[i][j];
		...
		if(dp[i+1][j+1]==Integer.MIN_VALUE) {
			ans3=minCostPaths(cost, i+1, j+1);
			dp[i+1][j+1]=ans3;
		}
		else {
			ans3=dp[i+1][j+1];
...
//        int ans=minCostPathsR(cost, 0, 0, dp); 
